    ...
    <header className=" fixed z-50 w-screen  p-3 px-4 md:p-6 md:px-16 bg-primary">
          ...
          />
          {
            isMenu && (
              <motion.div initial={{ opacity: 0, scale: 0.6 }}
                ...
                {
                  user && user.email === 'csoperativo@gmail.com' && (<Link to={"/createItem"} >
                ...
                <ul
                  ...
                  <li className="text-base text-headingColor cursor-pointer hover:text-headingColor duration-100 transition-all ease-in-out hover:bg-slate-100 px-4 py-2" onClick={ () => setIsMenu(false)}>
                    Home
                  </li>
                  <li className="text-base text-headingColor cursor-pointer hover:text-headingColor duration-100 transition-all ease-in-out hover:bg-slate-100 px-4 py-2" onClick={ () => setIsMenu(false)}>
                    Menu
      ...
      <div className="hidden md:flex w.full h-full items-center justify-between">
        ...
        <div className="flex items-center gap-8">
          ...
          <motion.div whileTap={{ scale: 0.6 }} className="relative flex items-center justify-center" onClick={showCart}>
            <MdShoppingBasket className="text-textColor text-2xl cursor-pointer" />
            {cartItems && cartItems.length > 0 && (<div className="absolute -top-2 -right-2 w-5 h-5 rounded-full bg-cartNumBg flex items-center justify-center">
            <p className="text-xs text-white  font-semibold">{cartItems.length}</p>
          </div>)}
